    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for qid in qids:
            ...
            while len(scores) < self.num_results and any(r for r in res_map):
                ...
                if 'docno' not in batch.columns:
                    logger.error(f"Critical: 'docno' missing from scorer output! Output Columns: {batch.columns}")
                    logger.error(f"Input batch columns: {batch_before.columns}")
                    logger.error(f"Output Head: {batch.head()}")
                    raise KeyError("docno column missing from scorer output")
